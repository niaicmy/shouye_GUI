...
def target_mkdir():
    ...
    try:
        ...
        if not os.path.exists(file_path):
            ...
            print("数据目录新建成功：" + file_path)
            return file_path
        else:
            print("数据目录已存在：" + file_path)
            return file_path
    except BaseException as msg:
        print("数据新建目录失败：" + str(msg))
...
def make_excel(source_path, staff_records_path, sql_config_path):
    ...
    :param source_path: 从数据库提取的原始数据文件路径
    :param staff_records_path: 员工档案文件路径
    :param sql_config_path: sql配置文件路径
    :return:
    """
    ...
    # 获取数据库 emr 库连接参数
    sql_config_p = load_json_file(sql_config_path)
    emr_config = sql_config_p['emr_config']
    # 打开数据库 获取游标
    ...
    target_file_name = target_data_path + '\\' + str(personal_data[0]) + str(personal_data[1]) + r'.xlsx'
...
if __name__ == '__main__':
    ...
    staff_records_p = os.getcwd() + "\\" + staff_records_name

    # 读取出院数据 生成病案文件
    # source_file_name = r'source_data.xlsx'
    # source_file_name = r'202105shouye.xlsx'
    source_file_name = r'12月病案首页.xlsx'
    source_data_path = os.getcwd() + "\\" + source_file_name

    # 数据库文件 路径
    sql_config_name = r'database_config.json'
    sql_config = os.getcwd() + "\\" + sql_config_name

    make_excel(source_data_path, staff_records_p, sql_config)
